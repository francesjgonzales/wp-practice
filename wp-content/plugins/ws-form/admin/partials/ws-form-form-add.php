...
?><li><a href="<?php esc_attr_e(sprintf('#wsf_template_category_%s', $template_category->id)); ?>"><?php esc_html_e($template_category->label); ?><?php

		if(($action_id !== false) && ($template_category->reload)) {

?><span data-action="wsf-api-reload" data-action-id="<?php esc_html_e($action_id); ?>" data-method="lists_fetch"<?php
...
<?php
	...
	foreach ($template_categories as $template_category)  {
		...
		if(isset($template_category->templates) && (count($template_category->templates) == 0)) { continue; }
?>
<!-- Tab Content: <?php esc_html_e($template_category->label); ?> -->
<div id="<?php esc_attr_e(sprintf('wsf_template_category_%s', $template_category->id)); ?>"<?php if(isset($template_category->action_id)) { ?> data-action-id="<?php esc_attr_e($template_category->action_id); ?>"<?php } ?><?php if(isset($template_category->action_list_sub_modal_label)) { ?> data-action-list-sub-modal-label="<?php esc_html_e($template_category->action_list_sub_modal_label); ?>"<?php } ?> style="display: none;">
...
<div id="wsf-form-add-loading" class="wsf-form-popup-progress">
	...
	<div class="wsf-form-popup-progress-inner"><img src="<?php esc_attr_e(sprintf('%sadmin/images/loader.gif', WS_FORM_PLUGIN_DIR_URL)); ?>" class="wsf-responsive" width="256" height="256" alt="<?php esc_html_e('Your form is being created...', 'ws-form'); ?>" /><p><?php esc_html_e('Your form is being created...', 'ws-form'); ?></p></div>
</div>
<!-- /Loading -->
...
<div class="wsf-modal-title"><?php
	...
	echo WS_Form_Common::get_admin_icon('#002e5f', false);
