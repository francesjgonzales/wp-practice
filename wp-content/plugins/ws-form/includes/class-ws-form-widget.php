<?php
	...
	class WS_Form_Widget extends WP_Widget {
		...
		public function form($instance) {
			...
			if($forms) {
?>
<p><label for="<?php esc_attr_e($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'ws-form'); ?></label> 
<input class="widefat" id="<?php esc_attr_e($this->get_field_id('title')); ?>" name="<?php esc_attr_e($this->get_field_name('title')); ?>" type="text" value="<?php esc_attr_e($title); ?>" /></p>

<p><label for="<?php esc_attr_e($this->get_field_id('form_id')); ?>"><?php esc_html_e('Select the form you want to add...', 'ws-form'); ?></label>

<select id="<?php esc_attr_e($this->get_field_id('form_id')); ?>" name="<?php esc_attr_e($this->get_field_name('form_id')); ?>">
<option value=""><?php esc_html_e('Select...', 'ws-form'); ?></option>
<?php
				foreach($forms as $form) {

?><option value="<?php esc_attr_e($form['id']); ?>"<?php if($form['id'] == $form_id) { ?> selected<?php } ?>><?php
					esc_html_e(sprintf('%s (ID: %u)', $form['label'], $form['id']));
?></option>
<?php
				}
?>
